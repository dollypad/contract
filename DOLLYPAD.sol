...
contract DOLLYPAD is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "DOLLYPAD";
    string constant _symbol = "DOLLY";
    uint8 constant _decimals = 18;

    uint256 _totalSupply = 100_000_000 * (10 ** _decimals);
    uint256 public _maxTxAmount = _totalSupply.div(50); // 2%
    uint256 public _maxWalletAmount = _totalSupply.div(50); // 2%
    ...
    uint256 public reflectionFee = 400; // 2%
    uint256 public marketingFee = 200; // 2% 
    uint256 public devFee = 200; // 2% 
    uint256 public totalFee = liquidityFee+buybackFee+reflectionFee+marketingFee+devFee;
    uint256 public feeDenominator = 10000; // Leave this Here.
    ...
    constructor (address _autoLiquidityReceiver, address _marketingFeeReceiver, address _devFeeReceiver) Auth(msg.sender) {
        ...
        isMaxLimitExempt[DEAD] = true;
        isMaxLimitExempt[address(this)] = true;
...
//        emit Transfer(sender, recipient, amount);
    ...
    function setFees(uint256[] memory _configFee, uint256 _feeDenominator) external authorized {
        ...
        devFee = _configFee[4];
        totalFee = liquidityFee.add(buybackFee).add(reflectionFee).add(marketingFee).add(devFee);
