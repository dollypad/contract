...
contract DollyBorrow is ReentrancyGuard, Ownable {
    using SafeMath for uint256;
	...
	function getBorrowId() view public returns (uint256) {
        return borrowId;
    }
	...
	function getBorrowData(uint256 _bid) view public returns (
		address, 
		address, 
		address, 
		uint256, 
	   ...
	   return(
			...
			,borrowData[_bid].collateralToken
			,borrowData[_bid].borrowToken
			,borrowData[_bid].collateralAmount
			,borrowData[_bid].borrowAmount
			,borrowData[_bid].interestRate
		);
    }
	
	function getBorrowPeriod(uint256 _bid) view public returns (
		uint256, 
		uint256, 
		uint256, 
		uint256,
	...
	function applyBorrow(
        ...
        borrowData[_id].collateralToken = collateralToken;
        borrowData[_id].borrowToken = borrowToken;
		borrowData[_id].collateralAmount = collateralAmount;
		borrowData[_id].borrowAmount = borrowAmount;
		borrowData[_id].interestRate = interestRate;
		borrowData[_id].borrowPeriod = borrowPeriod;
		borrowData[_id].borrowBroadcast = now;
		
		allBorrowIds.push(_id);
        borrowsByAddress[account].push(_id);
		walletTokenBalance[collateralToken][account] += collateralAmount;
		tokenBalance[collateralToken] += collateralAmount;
	...
	function cancelBorrow(
		...
		require(borrowData[_bid].borrowAmount > 0, 'cant lend, Borrow is not found');
		...
		walletTokenBalance[borrowData[_bid].collateralToken][borrowData[_bid].account] -= borrowData[_bid].collateralAmount;
		tokenBalance[borrowData[_bid].collateralToken] -= borrowData[_bid].collateralAmount;
	...
	function applyLend(
		...
		borrowData[_bid].borrowStart = block.timestamp;
		borrowData[_bid].borrowEnd = block.timestamp + borrowData[_bid].borrowPeriod;
	}
