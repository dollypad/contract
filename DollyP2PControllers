...
contract DollyP2PControllers is ReentrancyGuard, Ownable {
    using SafeMath for uint256;
	...
	DollyPlatformShare public _platformShareAddress;
	...
	uint256 public maxPeriod = 180; // days
	uint256 public minInterestRate = 1; // div by 10000
	uint256 public maxInterestRate = 10; // div by 10000
	...
	uint256 public platformShareFee = 7000;  // div by 10000
	...
	event FinishBorrow(uint256 indexed bid,bool withSell,bool withTakeOver,uint256 interestFee,uint256 borrowFeeAmount,uint256 borrowPayment);
	event FinishLend(uint256 indexed lid,bool withSell,bool withTakeOver,uint256 interestFee,uint256 lendFeeAmount,uint256 lendPayment);
    ...
    constructor (
		...
		_dolly = Dolly(dolly);
		_platformShareAddress = DollyPlatformShare(platformShareAddress);
	...
	function setLendTokenStatus(
		...
		if(lendTokenData[lendToken].index > 0) {
			...
			lendTokenData[lendToken].minBorrow = minBorrow;
			lendTokenData[lendToken].maxBorrow = maxBorrow;
			lendTokenData[lendToken].minSwap = minSwap;
		} else {
	...
	function applyBorrow(
		...
		if(borrowHoldTokenAmount > 0) {
			userBalance = IBEP20(address(_dolly)).balanceOf(msg.sender);
			userBalance = _getReverseTokenAmount(address(_dolly), userBalance);
			require(userBalance >= borrowHoldTokenAmount, 'Hold Balance insufficient');
		}
		
		
		require(borrowAmount >= lendTokenData[lendToken].minBorrow, 'not meet the min borrow amount');
		require(borrowAmount <= lendTokenData[lendToken].maxBorrow, 'not meet the max borrow amount');
		...
		if(collateralTokenData[collateralToken].thisBNB){
			...
			require(collateralAmount == msg.value, 'Collateral Amount not match');
		} else {
			userBalance = IBEP20(collateralToken).balanceOf(msg.sender);
		}
		...
		if(collateralTokenData[collateralToken].thisBNB){
			...
			if(collateralToken == address(_dolly)){
				_dolly.setIsFeeExempt(msg.sender,true);
			}
			
			uint256 balanceBeforeSendCollateral = IBEP20(collateralToken).balanceOf(address(this));
			IBEP20(collateralToken).safeTransferFrom(msg.sender, address(this), realCollateralAmount);
			...
			if(collateralToken == address(_dolly)){
				_dolly.setIsFeeExempt(msg.sender,false);
			}
		}
	...
	function applyLend(
		...
		require(lendAmount >= lendTokenData[lendToken].minBorrow, 'not meet the min lend amount');
		require(lendAmount <= lendTokenData[lendToken].maxBorrow, 'not meet the max lend amount');
		...
		if(lendTokenData[lendToken].thisBNB){
			...
			userBalance = IBEP20(lendToken).balanceOf(msg.sender);
		}
		...
		if(lendTokenData[lendToken].thisBNB){
			...
			if(lendToken == address(_dolly)){
				_dolly.setIsFeeExempt(msg.sender,true);
			}
			
			uint256 balanceBeforeSendLend = IBEP20(lendToken).balanceOf(address(this));
			IBEP20(lendToken).safeTransferFrom(msg.sender, address(this), realLendAmount);
	...
	function applyBorrowFromLend(
		...
		if(borrowHoldTokenAmount > 0) {
			userBalance = IBEP20(address(_dolly)).balanceOf(account);
			userBalance = _getReverseTokenAmount(address(_dolly), userBalance);
			require(userBalance >= borrowHoldTokenAmount, 'Hold Balance insufficient');
		}
		...
		if(collateralTokenData[lend.collateralToken].thisBNB){
			...
			if(lend.collateralToken == address(_dolly)){
				_dolly.setIsFeeExempt(msg.sender,true);
			}
	...
	function getInterestFee(uint256 _bid) public view returns (uint256 interestFee){
		...
		if(now > borrowStart){
			...
			interestFee = (borrowAmount * interestRate / _divRate) * ((period + 3600) / 3600);
		} else {
			interestFee = 0;
		}	
	}
	...
	function payLoan(
		...
		require(msg.sender == borrow.account, 'you are not Borrower Address');	
		...
		if(lendTokenData[lendToken].thisBNB){
			userBalance = msg.value;
			require(userBalance == loan.realLoanAmount, 'Balance not match');
		...
		if(loan.realFeeAmount > 0){
			sendFee(lendToken,loan.realFeeAmount);
		}
		
		_borrowAddress.finishBorrow(_bid);
		_lendAddress.finishLend(_lid);
		
		emit FinishBorrow(_bid, false, false, interestFee, borrowFeeAmount, now);
		emit FinishLend(_lid, false, false, interestFee, lendFeeAmount, now);
	}
	...
	function payLoanWithSell(
		...
		require(msg.sender == borrow.account || msg.sender == lender, 'you are not Borrower or Lender');	
		...
		emit FinishBorrow(_bid, true, false, interestFee, borrowFeeAmount, now);
		emit FinishLend(_lid, true, false, interestFee, lendFeeAmount, now);
	}
	
	function payLoanWithTakeOver(
		uint256 _bid
	) external payable {
		BorrowData memory borrow;
		...
		require(borrow.collateralAmount >= (currentCollateralAmount * 50 / 100), 'Not meet the requirement for forced sale collateral');	
		
		uint256 _lid = borrowFromLendData[_bid];
		(address lender,,,,) = _lendAddress.getLendData(_lid);
		
		require(msg.sender == borrow.account || msg.sender == lender, 'you are not Borrower or Lender');	
		
		
		sendCollateralToken(borrow.collateralToken, _getTokenAmount(borrow.collateralToken, borrow.collateralAmount), lender);
				
		_borrowAddress.finishBorrow(_bid);
		_lendAddress.finishLend(_lid);
		
		emit FinishBorrow(_bid, true, false, 0, 0, now);
		emit FinishLend(_lid, true, false, 0, 0, now);
	}
	...
	function sellToken(address lendToken, address collateralToken, uint256 collateralAmount) internal returns (uint256 userBalance){
		...
		if(collateralToken == _wbnb || lendToken == _wbnb){
			...
			if(collateralToken == _wbnb){
				_pancakeRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{value: tokensToSwap}(0, path, address(this), deadline);
			} else {
				_pancakeRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(tokensToSwap, 0, path, address(this), deadline);
			}
		} else {
	...
	function sendFee(address lendToken, uint256 feeAmount) internal {
		lendTokenData[lendToken].totalFee += feeAmount;
		
		if(lendTokenData[lendToken].totalFee > lendTokenData[lendToken].minSwap){
			uint256 feeBalance = 0;
			if(lendToken == _wbnb){		
				feeBalance = lendTokenData[lendToken].totalFee;
			} else {
				uint256 deadline = block.timestamp + 5 minutes;
				uint256 balanceBeforeSwap = address(this).balance;
				uint256 tokensToSwap = lendTokenData[lendToken].totalFee;
				
				IBEP20(lendToken).safeApprove(address(_pancakeRouter), 0);
				IBEP20(lendToken).safeApprove(address(_pancakeRouter), tokensToSwap);
				...
				feeBalance = balanceAfterSwap - balanceBeforeSwap;
			}
			
			uint256 platformShareAmount = feeBalance * platformShareFee / _divRate;
			uint256 teamShareAmount = feeBalance - platformShareAmount;
			
			_platformShareAddress.deposit{value: platformShareAmount}();
			payable(_teamAddress).transfer(teamShareAmount);
			
			lendTokenData[lendToken].totalFee = 0;
		}
	}
	...
	function _getCollateralAmount(
		...
		collateralAmount = collateralAmount * 100 / 60;
