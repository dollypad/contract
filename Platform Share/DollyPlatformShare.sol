...
contract DollyPlatformShare is ReentrancyGuard, Ownable {
    using SafeMath for uint256;
	...
	constructor (address _distributor) public Ownable() {
	...
	function deposit() external payable {
        ...
        dividendsPerShare = dividendsPerShare.add(dividendsPerShareAccuracyFactor.mul(msg.value).div(distributor.totalShares()));
    }
	...
	function getUnpaidEarnings(address shareholder) public view returns (uint256) {
        (uint256 amount,,) = distributor.shares(shareholder); 
	...
	function distributeDividend(address shareholder) internal {
       (shares[shareholder].amount,,) = distributor.shares(shareholder); 
